...
class TLSmarty extends Smarty
{
  ...
  {
  	$iconCfg = config_get('icon'); //icon list is moved in config.inc.php, and custom_config.inc.php
  	
  	foreach ($iconCfg as $key => $value)
  	{
  		$iconCfg[$key] = TL_THEME_IMG_DIR . $value;
  	}
  	//$iconCfg2 = array_map("icon_path", $iconCfg); //doesnt' work
