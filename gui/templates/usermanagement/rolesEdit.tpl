  ...
  <table class="common">
    <tr><th>{$labels.th_rolename}
      {if $gui->mgt_view_events eq "yes" && $gui->role->dbID}
        <img style="margin-left:5px;" class="clickable" src="{$tlImages.info}" onclick="showEventHistoryFor('{$gui->role->dbID}','roles')" alt="{lang_get s='show_event_history'}" title="{lang_get s='show_event_history'}"/>
      {/if}
    </th></tr>
    <tr><td>
