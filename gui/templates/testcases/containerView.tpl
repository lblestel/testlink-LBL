...
{if $gui->level == 'testproject'}
  ...
  <div id="tproject_control_panel" style="display:{$tlCfg->gui->op_area_display->test_spec_container};">
    ...
    <form method="post" action="lib/testcases/containerEdit.php">
      ...
      <input type="image" src="{$tlImages.specs_tso_sort_alpha}" name="reorder_testproject_testsuites_alpha" 
      ...
      <img src="{$tlImages.specs_tso_import}" onclick="location='{$importToTProjectAction}'" title="{$labels.btn_import_testsuite}" />

      {if $gui->canDoExport}
        <img src="{$tlImages.specs_tso_export}" onclick="location='{$tsuiteExportAction}'" title="{$labels.btn_export_all_testsuites}" />
      {/if}

      <img src="{$tlImages.specs_tso_report}" onclick="window.open('{$testSpecFullDocAction}')" 
           title="{$labels.btn_gen_test_spec_new_window}" />

      <img src="{$tlImages.specs_tso_word}" onclick="window.open('{$testSpecFullWordDocAction}')" 
