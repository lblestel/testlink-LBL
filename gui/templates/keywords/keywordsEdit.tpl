...
{if $canManage ne ""}
  ...
  <div class="action_descr">{$action_descr|escape}
  	{if $mgt_view_events eq "yes" && $keywordID > 0}
			<img style="margin-left:5px;" class="clickable" src="{$tlImages.info}" onclick="showEventHistoryFor('{$keywordID}','keywords')" alt="{lang_get s='show_event_history'}" title="{lang_get s='show_event_history'}"/>
	{/if}
